...
class AzureRMReplicationAlertSettings(AzureRMModuleBaseExt):
    def __init__(self):
        self.module_arg_spec = dict(
            ...
            name=dict(
                type='str',
                updatable=False,
                disposition='alertSettingName',
                required=true
            ),
            send_to_owners=dict(
                type='str',
                disposition='/properties/sendToOwners'
            ),
            custom_email_addresses=dict(
                type='list',
                disposition='/properties/customEmailAddresses'
            ),
            locale=dict(
                type='str',
    ...
    def exec_module(self, **kwargs):
        ...
        if not old_response:
            self.log("ReplicationAlertSetting instance doesn't exist")
            ...
            self.log('ReplicationAlertSetting instance already exists')
        ...
        if (self.to_do == Actions.Create) or (self.to_do == Actions.Update):
            self.log('Need to Create / Update the ReplicationAlertSetting instance')
            ...
            self.log('ReplicationAlertSetting instance deleted')
            self.results['changed'] = True
            ...
            self.log('ReplicationAlertSetting instance unchanged')
    ...
    def create_update_resource(self):
        # self.log('Creating / Updating the ReplicationAlertSetting instance {0}'.format(self.))
    ...
    def get_resource(self):
        ...
        try:
            ...
            # self.log("ReplicationAlertSetting instance : {0} found".format(response.name))
        except CloudError as e:
            self.log('Did not find the ReplicationAlertSetting instance.')
        if found is True:
            return response

...
class AzureRMReplicationvCenters(AzureRMModuleBaseExt):
    def __init__(self):
        self.module_arg_spec = dict(
            ...
            ip_address=dict(
                type='str',
                disposition='/properties/ipAddress'
            ),
            process_server_id=dict(
                type='str',
                disposition='/properties/processServerId'
            ),
            port=dict(
                type='str',
            ...
            run_as_account_id=dict(
                type='str',
                disposition='/properties/runAsAccountId'
            ),
            state=dict(
                type='str',
                default='present',
    ...
    def exec_module(self, **kwargs):
        ...
        self.url = self.url.replace('{{ replicationv_center_name }}', self.name)

        old_response = self.get_resource()

        if not old_response:
            self.log("ReplicationvCenter instance doesn't exist")
            ...
            self.log('ReplicationvCenter instance already exists')
        ...
        if (self.to_do == Actions.Create) or (self.to_do == Actions.Update):
            self.log('Need to Create / Update the ReplicationvCenter instance')
            ...
            self.log('ReplicationvCenter instance deleted')
            self.results['changed'] = True
            ...
            self.log('ReplicationvCenter instance unchanged')
    ...
    def create_update_resource(self):
        # self.log('Creating / Updating the ReplicationvCenter instance {0}'.format(self.))
    ...
    def get_resource(self):
        ...
        try:
            ...
            # self.log("ReplicationvCenter instance : {0} found".format(response.name))
        except CloudError as e:
            self.log('Did not find the ReplicationvCenter instance.')
        if found is True:
            return response

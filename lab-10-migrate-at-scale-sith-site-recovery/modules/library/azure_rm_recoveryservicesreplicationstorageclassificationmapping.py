...
class AzureRMReplicationStorageClassificationMappings(AzureRMModuleBaseExt):
    def __init__(self):
        self.module_arg_spec = dict(
            ...
            storage_classification_name=dict(
                type='str',
                updatable=False,
                disposition='storageClassificationName',
                required=true
            ),
            name=dict(
                type='str',
                updatable=False,
                disposition='storageClassificationMappingName',
                required=true
            ),
            target_storage_classification_id=dict(
                type='str',
                disposition='/properties/targetStorageClassificationId'
            ),
            state=dict(
                type='str',
                default='present',
    ...
    def exec_module(self, **kwargs):
        ...
        if not old_response:
            self.log("ReplicationStorageClassificationMapping instance doesn't exist")
            ...
            self.log('ReplicationStorageClassificationMapping instance already exists')
        ...
        if (self.to_do == Actions.Create) or (self.to_do == Actions.Update):
            self.log('Need to Create / Update the ReplicationStorageClassificationMapping instance')
            ...
            self.log('ReplicationStorageClassificationMapping instance deleted')
            self.results['changed'] = True
            ...
            self.log('ReplicationStorageClassificationMapping instance unchanged')
    ...
    def create_update_resource(self):
        # self.log('Creating / Updating the ReplicationStorageClassificationMapping instance {0}'.format(self.))
    ...
    def get_resource(self):
        ...
        try:
            ...
            # self.log("ReplicationStorageClassificationMapping instance : {0} found".format(response.name))
        except CloudError as e:
            self.log('Did not find the ReplicationStorageClassificationMapping instance.')
        if found is True:
            return response
